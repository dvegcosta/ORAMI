...
import { BUCKETS, removerImagemStorage } from './storage';
...
export default function PostCard({
  ...
  onOcultarSuccess,
  idComunidade,
  ehModeradorNaComunidade = false,
  variant = 'padrao',
}) {
  const isAdmin = variant === 'comunidade';
  const podeOcultarNaComunidade = isAdmin && Boolean(ehModeradorNaComunidade && idComunidade);
  ...
  const handleOcultarPost = () => {
    setMenuVisivel(false);
    Alert.alert(
      'Ocultar Post',
      'Tem certeza que deseja ocultar esta publicação desta comunidade?',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const { data, error } = await supabase.rpc('orami_ocultar_post_comunidade', {
                p_id_comunidade: idComunidade,
                p_id_post: item.id_post,
                p_id_usuario: idUsuario,
              });
              if (error) throw error;
              if (!data?.sucesso) throw new Error(data?.mensagem || 'Ação não autorizada');
              if (data?.post_excluido === true) {
                await removerImagemStorage(item.imagem_base64 || item.img_post, BUCKETS.POSTS);
              }
              onOcultarSuccess?.(item.id_post, data?.post_excluido === true);
            } catch (error) {
              console.error('Erro ao ocultar postagem da comunidade:', error);
              Alert.alert('Erro', error?.message || 'Não foi possível ocultar a publicação desta comunidade.');
            }
          },
        },
      ]
        ...
        <View style={{ position: 'relative', zIndex: 999 }}>
          ...
          {menuVisivel && (
            <View style={{ position: 'absolute', right: 25, top: 0, backgroundColor: cores.superficie || '#FFF', borderRadius: 12, padding: 5, elevation: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 8, minWidth: 180, zIndex: 1000 }}>
              {podeOcultarNaComunidade ? (
                <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', padding: 12 }} onPress={handleOcultarPost}>
                  <Ionicons name="eye-off-outline" size={20} color={cores.aviso} />
                  <Text style={{ marginLeft: 8, color: cores.aviso, fontWeight: 'bold', fontFamily: 'REM_Regular' }}>Ocultar Post</Text>
                </TouchableOpacity>
              ) : postEhMeu ? (
